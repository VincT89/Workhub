...
const DrawerSede = ({ open, onClose, productData }) => {
 ...
 const searchStores = () => { 
    ...
    */
    ...
    const product = productData.find( // trovo il prodotto in productData
...
*/}
          ...
          <button
            ...
            className="px-4 py-2 bg-[#d5c7ff]/70 border border-white/70 shadow-sm rounded-lg text-sm text-[#090c64] hover:bg-[#cfc0ff] mb-6"
          >
            Cerca disponibilità
          </button>
          ...
          <div>
            ...
            {results.map((item, i) => {
              if (item.error)
                return <p key={i} className="text-red-600">{item.error}</p>;

              const [sede, qty] = item;

              return (
                <div key={i} className="py-2 border-b border-white/40 flex justify-between">
                  <span>{sede}</span>
