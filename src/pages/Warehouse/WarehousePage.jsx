...
const WarehousePage = () => {
	...
	const textColor = theme === "dark" ? "text-white" : "text-[#134a7b]"; // determina il colore del testo in base al tema corrente
	const subTextColor = theme === "dark" ? "text-gray-300" : "text-gray-600"; // determina il colore del sottotesto in base al tema corrente

	// ---- CREO L'ARRAY PER RENDERE I BOX DINAMICI ----
	const buttons = [
		{ label: t("warehouse.prodottiTotali"), number: 240, icon: packageIcon },
		{
			label: t("warehouse.ordiniInUscita"),
			number: 32,
		...
		<>
			...
			>
				...
				<div
					...
					rounded-[25px] p-6 shadow-md flex flex-col gap-2 cursor-pointer transition-all duration-200 mx-1
					`}
				>
					{/* Header tabella */}
					<div className={`grid grid-cols-6 font-bold text-sm ${textColor}`}>
					...
					<div
						className="h-64 pr-2 space-y-2 overflow-y-auto 
						scrollbar-thin scrollbar-thumb-[#1C62A0] scrollbar-track-transparent 
						scrollbar-thumb-rounded-full scrollbar-track-rounded-full
						hover:scrollbar-thumb-[#155293] transition-colors duration-300"
					>
						{products.map((p) => (
							<Link
								key={p.id}
								to={`/product/${p.id}`}
								className="grid grid-cols-6 bg-white/40 dark:bg-[#fafafa30]
								rounded-full p-2 shadow-sm transition duration-200 
								hover:bg-white/70 dark:hover:bg-[#1C62A0]/20 cursor-pointer"
							>
								<span>{p.id}</span>
								<span>{p.nome}</span>
				...
				<div className="flex gap-4 mb-3">
					{/* Depositi */}
					<div
						className={`flex-1 bg-[#fafafa20] dark:bg-[#fafafa30] backdrop-blur-sm border border-white/30 dark:border-white/40 
						rounded-[25px] p-6 shadow-md cursor-pointer transition-all duration-200 mx-1
						`}
					>
						<div className="flex items-center gap-2 mb-4">
							<img src={warehouseIcon} alt="Depositi" className="w-6 h-6" />
							<h2 className={`text-lg font-bold ${textColor}`}>
								{t("warehouse.depositi")}
							</h2>
						</div>
						<div className="grid grid-cols-6 bg-white/40 dark:bg-[#fafafa30] rounded-full p-2 shadow-sm">
							<span
								className={`col-span-6 w-full whitespace-nowrap overflow-hidden text-ellipsis ${subTextColor}`}
							>
					...
					<div className="flex-1 bg-[#fafafa20] dark:bg-[#fafafa30] backdrop-blur-sm border border-white/30 dark:border-white/40 
					rounded-[25px] p-6 shadow-md">
						<div className="flex items-center gap-2 mb-4">
							<img
								src={ideaIcon}
								alt="Icona suggerimenti"
								className="w-6 h-6"
							/>
							<h2 className={`text-lg font-bold ${textColor}`}>
								{t("warehouse.suggerimenti")}
							</h2>
						</div>
						<div className="grid grid-cols-6 bg-white/40 dark:bg-[#fafafa30] rounded-full p-2 shadow-sm">
							<span
