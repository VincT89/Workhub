...
const Product = () => {
    ...
    */
	...
	return (
		<>
			<div className="w-full h-full flex flex-col gap-8 overflow-y-auto custom-scrollbar-invisible pr-2">
				{/* --------- SEZIONE 1: BOX RIASSUNTIVI --------- */}
				<section className="section-base grid-4">
					{buttons.map((btn, index) => (
						<div key={index} className={`widget-box glass-card ${textColor}`}>
					...
					<div className="flex items-center justify-between mb-4">
						<div className="flex items-center gap-3">
							...
							{prodotto.img ? (
								...
								<div className="w-12 h-12 rounded-full bg-white/30 flex items-center justify-center text-sm text-primary dark:text-(--text-dark)">
									Img
								</div>
							)}
							<div className="flex items-center gap-2">
						...
						<div className="flex flex-col items-center justify-center h-full w-full">
							{prodotto.img ? (
								<img
									src={prodotto.img}
									alt={prodotto.nome}
									className="w-full h-full max-h-80 object-contain rounded-2xl shadow-md bg-white/30 dark:bg-glass-strong"
								/>
							) : (
								<div className="w-full h-80 rounded-2xl bg-white/30 flex items-center justify-center text-sm text-primary dark:text-(--text-dark) shadow-md">
									Nessuna immagine
								</div>
							)}
						</div>
                ...
                */}
				...
				<div className="glass-card p-6 shadow-md">
					...
					{storico.map((m, i) => (
						<div
							key={i}
							className="grid grid-cols-3 bg-white/40 dark:bg-glass-strong rounded-full p-2 shadow-sm mb-2"
						>
							<span>{m.data}</span>
							<span>{m.tipo}</span>
