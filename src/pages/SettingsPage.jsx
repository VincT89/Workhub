...
const SettingsPage = () => {
	...
	// 🎨 Colori coerenti con LoginPage e dark mode
	const textColor =
		theme === "dark"
			? "text-[var(--text-dark)]"
		...
		<main className="main-container bg-white dark:bg-black transition-colors duration-500">
			...
			<div className="relative z-20 w-full max-w-[800px] flex flex-col items-center px-6 py-10 space-y-10">
				...
				<section className="w-full glass-card p-6 shadow-sm">
					<h2
						className={`${textColor} text-xl font-bold mb-4 border-b border-primary pb-2`}
					>
						{t("settings.account")}
					</h2>
					...
					{message && (
						<p
							className={`${
								messageType === "success" ? "text-primary" : "text-red-500"
					...
					<form onSubmit={handleSave} className="flex flex-col gap-4">
						{/* Username */}
						<div>
							<label className={`label-base ${labelColor}`}>
								{t("settings.username")}
							</label>
							<input
								type="text"
				...
				<section className="w-full flex flex-col sm:flex-row justify-center gap-6 mt-2">
					<button
						type="submit"
						onClick={handleSave}
						className="btn-primary w-full sm:w-[200px] py-3"
					>
						{t("settings.salvaModifiche")}
					</button>
					<Link
						to="/login"
						className="btn-primary w-full sm:w-[200px] py-3 text-center"
					>
						{t("settings.esci")}
					</Link>
				</section>
